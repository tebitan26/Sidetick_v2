...
export default function WaitlistForm({ referrer }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!consent) { setErr("Merci d'accepter de recevoir nos emails de lancement."); return; }
    if (honeypot.current && honeypot.current.value) { setErr("Oups, une erreur est survenue."); return; }
    ...
    try {
      ...
      const url = new URL("/waitlist/confirm", location.origin);
      if (referrer) url.searchParams.set("ref", referrer);
      url.searchParams.set("consent", consent ? "1" : "0");
      url.searchParams.set("src", "website");
      if (document.referrer) url.searchParams.set("came_from", document.referrer);

      // OTP Supabase (double opt-in)
