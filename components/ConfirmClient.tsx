...
export default function ConfirmClient({
  ...
  useEffect(() => {
    (async () => {
      try {
        // 1) Échanger le ?code= présent dans l’URL contre une session
        const { error: exErr } = await sb.auth.exchangeCodeForSession();
        ...
        const { data: myCode, error: rpcErr } = await sb.rpc(
          "confirm_waitlist",
          {
            p_email: email,
        ...
        const code =
          typeof myCode === "string" && myCode.length > 0 ? myCode : "";
